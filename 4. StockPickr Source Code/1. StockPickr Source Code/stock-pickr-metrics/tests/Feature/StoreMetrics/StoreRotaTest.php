...
class StoreRotaTest extends TestCase
{
    use RefreshDatabase;
    ...
    /** @test */
    public function it_should_store_rota()
    {
    ...
    /** @test */
    public function it_should_store_rota_as_negative()
    {
        $company = $this->createCompanyUpsertedContainer([
            'balanceSheets' => [
                'tangibleAssets' => [
        ...
        $this->assertEquals(-0.1, round($companyMetric->rota, 1));

        $companyMetric = $this->getCompanyMetric(2019);
        $this->assertEquals(-0.15, round($companyMetric->rota, 2));

        $companyMetric = $this->getCompanyMetric(2018);
        $this->assertEquals(-0.20, round($companyMetric->rota, 2));
    }
}
